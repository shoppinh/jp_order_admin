...
const SupportTopics = () => {
  ...
  return (
    <React.Fragment>
      ...
      {topic && (<Content>
        <BlockHead size="lg">
          <div className="nk-block-head-sub"><Link className="back-to" to={`${process.env.PUBLIC_URL}/subscription/support`}><Icon name="arrow-left"/><span>Supports</span></Link></div>
          <BlockBetween>
            <BlockHeadContent>
            ...
            <div className="card-inner">
              {supportQuestions.filter((item) => item.topic === topic.id).map((item,index)=> 
                <div className="support-queue-item" key={index}>
                    <Link className="support-queue-link" to={`${process.env.PUBLIC_URL}/subscription/support/${topic.slug}/${item.id}`}>
