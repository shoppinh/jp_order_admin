          ...
          {(search === "") ? supportTopics.map((item,index) =>{
            ...
            return(
              <div key={index} className="support-topic-item card card-bordered">
                <Link className="support-topic-block card-inner" to={count.length ? `${process.env.PUBLIC_URL}/subscription/support/${item.slug}` : `${process.env.PUBLIC_URL}/subscription/contact`}>
              ...
              <div className="card-inner">
                {(questions.length > 0) ? questions.map((item,index)=> 
                  <div className="support-queue-item" key={index}>
                      <Link className="support-queue-link" to={`${process.env.PUBLIC_URL}/subscription/support/general-topics/${item.id}`}>
