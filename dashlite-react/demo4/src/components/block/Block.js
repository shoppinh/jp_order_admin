...
export const BlockBetween = ({ className, size, ...props }) => {
  return !size ? <div className={`nk-block-between ${className ? className : ""}`}>{props.children}</div> : <div className={`nk-block-between-${size} ${className ? className : ""}`}>{props.children}</div>;
};
