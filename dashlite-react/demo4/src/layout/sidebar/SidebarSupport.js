    ...
    <div className={classes}>
      <SimpleBar className="nk-sidebar-menu p-3 p-lg-0">
        <div className="side-wg">
            <Link className="back-to" to={`${process.env.PUBLIC_URL}/subscription/`}><Icon name="home"></Icon><span>Back to Home</span></Link>
        </div>
        <div className="side-wg">
            ...
            <div className="side-wg-text">
                <ul className="list-plain list-category">
                    {supportTopics.map((item,index)=> 
                      <li key={index}>
                        <Link to={`${process.env.PUBLIC_URL}/subscription/support/${item.slug}`}>{item.title}</Link>
                      </li>
                    )}
                </ul>
            </div>
            ...
            <div className="side-wg-text">
                <ul className="list-plain">
                    {random && random.slice(0,4).map((item,index)=>{
                      ...
                      return(
                        <li key={index}><Link to={`${process.env.PUBLIC_URL}/subscription/support/${topicslug.slug}/${item.id}`}>{item.title}</Link></li>
                      )
                    })}
                </ul>
            </div>
