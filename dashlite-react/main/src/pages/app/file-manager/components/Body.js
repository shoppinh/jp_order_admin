...
const FilesBody = ({ searchBar, title, viewFilter, recoveryFilter, ...props }) => {
  ...
  const searchResult = [
    ...fileManager.files.filter(
      (item) => !item.deleted && item.name.toLowerCase().includes(fileManager.search.toLowerCase())
    ),
  ];
          ...
          <BlockBetween className='position-relative'>
            <BlockHeadContent>
              ...
              {fileManager.search !== '' && (
                <BlockTitle page>
                  Search for :{' '}
                  <span className='fw-normal ms-2 text-muted'>{fileManager.search}</span>
                </BlockTitle>
              )}
            </BlockHeadContent>
            <BlockHeadContent>
                  ...
                  <li className='d-lg-none'>
                    <UncontrolledDropdown>
                      ...
                      >
                        <Icon
                          name={
                            viewOptions.filter((item) => item.value === fileManager.filesView)[0]
                              .icon
                          }
                        ></Icon>
                      </DropdownToggle>
